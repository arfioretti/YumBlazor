...
namespace YumBlazor.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<IEnumerable<OrderHeader>> GetAllAsync(int? userId = null)
        {
            if (userId != null) 
            { 
                return await _db.OrderHeaders.Where(u=>u.Id==userId).ToListAsync();
            }
        ...
        {
            var orderHeader= await _db.OrderHeaders.FirstOrDefaultAsync(u=>u.Id==orderId); 
            if(orderHeader != null)
            {
